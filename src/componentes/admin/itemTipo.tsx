...
export function ItemTipo({ tipo }: ItemTipoProps) {
    ...
    return (
        <Tr textTransform={'capitalize'} h={65} key={tipo.id} _hover={{ backgroundColor: '#F4F4F4' }}>
            <Td>
                <Box display={'flex'} flexDir={'row'} gap={2}>
                    <Text fontWeight={500} m={'auto'}>{tipo.nomeDoTipo}</Text>
                </Box>
            </Td>
            <Td>
                <Box display={'flex'} flexDir={'row'} gap={2}>
                    <Text fontWeight={500} m={'auto'}>{tipo.created_at}</Text>
                </Box>
            </Td>
            <Td>
