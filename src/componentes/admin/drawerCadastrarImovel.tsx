...
import { newFormDataCreateImovel, validationSchemaCreateImovel } from "../../utils/schema";
...
export function DrawerCadastrarImovel({ isOpen, onClose }: ModalCadastrarImovelProps) {
    const { isOpen: isOpenTipo, onClose: onCloseTipo, onOpen: onOpenTipo } = useDisclosure();
    const { isOpen: isOpenCidade, onClose: onCloseCidade, onOpen: onOpenCidade } = useDisclosure();
    const { isOpen: isOpenAnexarImagem, onClose: onCloseAnexarImagem, onOpen: onOpenAnexarImagem } = useDisclosure();
    const { data } = useTipoAll();
    const { data: dataCidade } = useCidadeAll();
    const [files, setFiles] = useState<File[]>([]);
    const mutationCadastrarImovel = useMutationCreateImovel();
    const { register, handleSubmit, reset, getValues } = useForm<newFormDataCreateImovel>({
                ...
                <DrawerContent>
                    ...
                    <DrawerBody>
                        <FormControl isRequired mb={5}>
                            ...
                            <Select {...register('cidadeID')} mt={-1.5} focusBorderColor={'#49479D'}>
                        ...
                        <FormControl isRequired mb={5}>
                            ...
                            <Select {...register('tipoID')} mt={-1.5} focusBorderColor={'#49479D'}>
                                {data?.tipo.map((tipo) => (
                        ...
                        <FormControl mb={5}>
                            ...
                            <Input placeholder={'Se necessário informe a quantidade'} {...register('quantidadeQuarto')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
                        <FormControl mb={5}>
                            <FormLabel>Quantidade de Garagem</FormLabel>
                            <Input placeholder={'Se necessário informe a quantidade'} {...register('quantidadeGaragem')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
                        <FormControl mb={5}>
                            <FormLabel>Quantidade de Banheiros</FormLabel>
                            <Input placeholder={'Se necessário informe a quantidade'} {...register('quantidadeBanheiros')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
                        <FormControl isRequired mb={5}>
                            <FormLabel>Endereço</FormLabel>
                            <Input placeholder={'Informe o endereço'} {...register('endereco')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
                        <FormControl isRequired mb={5}>
                            <FormLabel>Valor</FormLabel>
                            <Input placeholder={'Informe o valor'} {...register('valor')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
                        <FormControl isRequired mb={5}>
                            <FormLabel>Número de Matrícula</FormLabel>
                            <Input placeholder={'Informe a matrícula'} {...register('numeroDaMatricula')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
                        <FormControl isRequired mb={5}>
                            <FormLabel>Área</FormLabel>
                            <Input placeholder={'Informe a área do imovél'} {...register('area')} mt={-1.5} focusBorderColor={'#49479D'} />
                        </FormControl>
